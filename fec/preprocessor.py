...
def same(key, c1, c2):
    match = 0
    if clean_str(c1[key]) == clean_str(c2[key]):
        match = 1
    return match

def similarity(key, c1, c2):
    if key == 'zipcode':
        counter = 0.0
        z1, z2 = [clean_str(x) for x in [c1['zipcode'], c2['zipcode']]]
        ...
        if clean_str(c1['zipcode'])[0] == clean_str(c2['zipcode'])[0]:
            return 1
        return 0

    elif key == 'zipcode_sectionalcenter':
        if clean_str(c1['zipcode'])[:3] == clean_str(c2['zipcode'])[:3]:
            return 1
        return 0

    else:
        c1_shingles = Preprocessor()._shingle(clean_str(c1[key]), 3)
        c2_shingles = Preprocessor()._shingle(clean_str(c2[key]), 3)
        return Preprocessor()._jaccard_sim(c1_shingles, c2_shingles)
...
def create_featurevector(c1, c2):
    ...
    for key in same_keys:
        features.append(same(key, c1, c2))
    ...
    for key in sim_keys:
        features.append(similarity(key, c1, c2))
    return features

class Preprocessor(object):

    def __init__(self, training_set_size=None):
        ...
        self.training_set_size = 1000000 if training_set_size == None else training_set_size

    def preprocess(self, path):
        self.path = path
        self._group_by_last_name()
        to_create = []
        for last_name, matches in self.last_name_groups.iteritems():
            ...
            for c in itertools.combinations(matches, 2):
                ...
                if self._jaccard_sim(self._shingle(compstring1.lower(), 2), self._shingle(compstring2.lower(), 2)) >= self.initial_sim:
                    c1, c2 = c[0], c[1]
        ...
        # Pickle matches
        pickle.dump(to_create, open('data/training_matches.p', 'w'))
    ...
    def _group_by_last_name(self):
        n=0
        for row in csv.reader(open(self.path), delimiter=',', quotechar='"'):
            n += 1
